  ...
  <el-card>
    <el-form label-width="120px">
      ...
      <el-table :header-cell-style="{ background: '#f0f5ff' }" :data="tableData" style="width: 100%;margin-top:0">
        ...
        <el-table-column prop="staffName" label="发布人"> </el-table-column>
        <el-table-column prop="noticeContent" label="公告内容" width="300">
        ...
        <el-table-column label="是否在首页显示">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isshow" :active-value="1" :inactive-value="0" @change="switchChange(scope.row.id)"> </el-switch>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="id >= 0 ? '编辑' : '新增'" :visible.sync="dialogVisible" width="30%" @close="dialogClose">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <el-form-item label="文件上传">
          <el-upload
            ...
            action="http://192.168.0.186:8080/fileController/upload"
...
export default {
  ...
  methods: {
    ...
    async downLoad(id) {
      ...
      if (res.code !== '0010') return this.$message.error(res.msg)
      window.location.href = 'http://192.168.0.186:8080/' + res.data
    },
    // 获取页面初始数据
    async getData() {
