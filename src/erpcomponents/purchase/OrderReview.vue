<template>
  <el-card>
    <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <template>
          ...
          <el-table :header-cell-style="{ background: '#f0f5ff' }" :data="tableData" style="width: 100%">
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button type="primary" size="mini" @click="toDetail(scope.row)" :disabled="rolePowerList.indexOf(61)==-1">查看</el-button>
              </template>
            </el-table-column>
          </el-table>

          <el-pagination :current-page="selectedForm.pageNum" :page-sizes="[5, 10, 15, 20]" :page-size="selectedForm.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
          </el-pagination>
        </template>
      </el-tab-pane>
      <el-tab-pane label="审核设置" name="second">
        <template>
          ...
          <el-table :header-cell-style="{ background: '#f0f5ff' }" :data="tableData" style="width: 100%">
            <el-table-column type="index" label="序号"> </el-table-column>
            <el-table-column prop="levelName" label="层次名称"> </el-table-column>
    ...
    <!-- 审核设置的新增对话框 -->
    <el-dialog :title="id >= 0 ? '编辑' : '新增'" :visible.sync="addDialogVisible" width="30%" @close="dialogClose">
