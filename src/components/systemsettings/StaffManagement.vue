...
export default {
    ...
    methods: {
        ...
        async del(row){
            ...
            const { data: res } = await this.$http.post('SysController/updateSysStateById',
            {"findModelName":this.activeName, "updateID":row.id, "updateType": "D"})
            if (res.code !== "0010"){
        ...
        async StateChange(row){
            ...
            const { data: res } = await this.$http.post('SysController/updateSysStateById',
            {"findModelName":this.activeName, "updateID":row.id, "updateType": "S"})
            if (res.code !== "0010"){
                return this.$message.error(res.msg)
            }else{
    ...
    //   点击新增按钮
      onSubmit() {
        switch (this.activeName) {
            case 'staff':
                this.$router.push('/staffManagement/addStaff')
                break;
            case 'department':
                this.$router.push('/staffManagement/addDepartment')
                break;
            case 'mesPost':
                this.$router.push('/staffManagement/addTitle')
                break;
            case 'tTeam':
                this.$router.push('/staffManagement/addClass')
                break;
            case 'sRole':
                this.$router.push('/staffManagement/addRole')
                break;
            default:
                break;
        }
