  ...
  <div>
    ...
    <el-card style="margin-top:20px;">
      <div style="font-weight:bold;">
        历史检查数据
        <el-button type="primary" style="margin-left:70%;" @click="tocpSetDetail()">新增维护</el-button>
      ...
      <el-table :data="cuffingcheck" style="width: 100%" :header-cell-style="{ background: '#f0f5ff' }">
        ...
        <el-table-column prop="measureddiamUm" label="实测直径"> </el-table-column>
        <el-table-column prop="staffName" label="检查人"> </el-table-column>
        <el-table-column prop="remark" label="备注"> </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" @click="tocpSetDetail(scope.row.id)">查看</el-button>
...
export default {
  ...
  methods: {
    ...
    async del(id) {
      ...
      const { data: res } = await this.$http.post('mouldController/updateFinishedproducecheckByid',{
        updateID:id
      })
      if (res.code !== "0010") return this.$message.error(res.msg)
    ...
    async getData() {
      const { data: res } = await this.$http.post('mouldController/getTMouldFinishedproducecheckById', {
        findById: this.id
      })
      if (res.code !== '0010') return this.$message.error(res.msg)
      this.tmould = res.data.tmould
      this.cuffingcheck = res.data.Finishedproducecheck
    },
    tocpSetDetail(id = -1) {
      if (id <= 0) {
        this.$router.push({
