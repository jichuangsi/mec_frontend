<template>
  <div>
    ...
    <el-card style="margin-top:20px">
        <div style="font-weight:bold;margin-bottom:20px;">审核流程</div>
        <el-timeline>
        ...
        <div class="footer" v-if="pp.ppPlanState==1||pp.ppPlanState==2">
            <el-button type="primary" @click="showDialog('T')">审核通过</el-button>
            <el-button @click="showDialog('F')">驳回</el-button>
        </div>
    </el-card>
    <el-dialog
    title="提示"
...
export default {
  ...
  methods: {
    ...
    //   确定保存
    async dialogConfirm(){
        const { data: res } = await this.$http.post('ProductionPlanController/updatePPStateByid',{
            updateID:this.id,
            updateRemark:this.remark,
