    ...
    <el-dialog title="设置班组和班次" :visible.sync="dialogClassVisible" width="40%">
      <el-form label-width="140px">
        <el-form-item label="工序">
          <el-input v-model="pp_schedulingItem.gxName" style="width:51%;" disabled></el-input>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogClassConfirm">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 关联销售订单对话框 -->
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-table :data="LData" highlight-current-row style="width: 100%" :cell-style="{ padding: '5px 0' }" :header-cell-style="{ background: '#f0f5ff', padding: '0' }" @row-click="rowClick">
              <el-table-column prop="mapValue" label="产品编号"> </el-table-column>
              <el-table-column prop="mapValue2" label="产品名称"> </el-table-column>
              <el-table-column prop="mapliandong" label="产品型号"> </el-table-column>
            </el-table>
          </el-col>
          <el-col :span="12">
  ...
  methods: {
    ...
    linkDialogConfirm() {
      this.multipleSelection.forEach((item, index) => {
        this.row.productDetailId = item.mapKey
        this.row.standards = item.mapValue
    ...
    async getData() {
      const { data: res } = await this.$http.post('ProductionPlanController/getPPBasicInfo', { findById: '' })
      if (res.code !== '0010') return this.$message.error(res.msg)
      this.GX = res.data.GX
